...
xhr.send("name="+imgElement.file.name+" "+imgElement.height+"x"+imgElement.width+"-->index\n");
    //#######################################################*/
  ...
  function loadBm() {
    ...
    //+++++++++++     eliminate  trasparenza ++++++++++++++++++++++++++++++
    bm = new Bitmap(imgCanvas.width, imgCanvas.height);
    var imgdataobj = ctx.getImageData(0, 0, bm.w, bm.h);
	for(var i=0;i<imgdataobj.data.length;i+=4){
    if(imgdataobj.data[i+3]<255){
        imgdataobj.data[i]=255;
        imgdataobj.data[i+1]=255;
        imgdataobj.data[i+2]=255;
        imgdataobj.data[i+3]=255;
    }
}
//++++++++++++++++++++ eliminate trasparenza sfarsit +++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  function bmToPathlist() {
    ...
    while (currentPoint = findNext(currentPoint)) {
      ...
      if (path.area > document.getElementById("stroke").value) {
        pathlist.push(path);
      }
    }
